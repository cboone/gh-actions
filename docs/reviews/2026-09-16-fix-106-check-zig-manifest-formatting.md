...
- [x] **Item 1, code-change:** Retain behavioral coverage in the repository's self-tests so later changes to the default paths or parsing cannot silently restore the gap. Resolved by `tests/scrut/zig-format-paths.md` and `tests/fixtures/check-zig-format-paths.mjs`, with a `zig-format-paths` matrix job in `run-ci.yml` for Linux amd64, Linux arm64, and macOS arm64. The fixture reads the production workflow's default, environment binding, shell, and formatting command and runs nine isolated scenarios. The YAML parser is an exact direct dependency installed through the integrity-pinned lockfile.

All nine retained scenarios pass locally with Zig 0.16.0. Temporarily removing the manifest from the default and reverting to the previous formatting command each makes the suite fail; the workflow was restored after those checks. The existing uv tests, workflow linting, YAML linting, Markdown linting, spelling, formatting, and diff checks also pass. Linux and hosted GitHub Actions execution were not exercised locally, and other Zig versions were not tested. Total actionable items: 1; resolved: 1; skipped: 0. Implementation commit: `6d66a06`.
