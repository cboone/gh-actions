...
jobs:
  ...
  secrets:
    uses: cboone/gh-actions/.github/workflows/scan-for-secrets.yml@v3.1.0
    with:
      tool: both
```
...
tags. Pin to a specific version (e.g., `@v3.1.0`) for production use.
...
exact version tag (e.g., `v3.1.0`) pointing to the release commit.
...
Always pin to an exact release tag (e.g. `@v3.1.0`). Branch refs like
