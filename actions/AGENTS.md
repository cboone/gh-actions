...
- Use `using: composite`. `set-up-*` installs; `run-*` adds execution. Accept a pinned version default unless the documented interface deliberately makes the caller supply the pin.
...
- Keep the generic installer Bash 3.2 compatible. Detect OS and architecture, reject unsupported systems, verify SHA-256, install to `RUNNER_TEMP`, then update `GITHUB_PATH`.
