...
- Ordinary data and argument inputs must reach shell commands through `env:` mappings, with quoted variables and explicit argument arrays. Explicit command inputs such as `scrut-setup-cmd`, `scrut-build-cmd` and `build-command` intentionally execute through `run:`; the `workflow-arg-binding` CI job enforces that allowlist. Release binaries install under `RUNNER_TEMP` and use `GITHUB_PATH` or an absolute invocation path; other trust paths follow the development reference. The generic installer must remain Bash 3.2 compatible.
